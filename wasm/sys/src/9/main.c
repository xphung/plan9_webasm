...
usage(void)
{
	kprint(2, "Usage: emu [options...] [file.dis [args...]]\n"
		"\t-gXxY\n"
		"\t-c[0-9]\n"
		"\t-d file.dis\n"
		"\t-s\n"
...
envusage(void)
{
  kprint(2, "emu: bad option in EMU environment variable (%s)\n", "");  //getenv("EMU"));
	usage();
}
...
option(int argc, char *argv[], void (*badusage)(void))
{
	...
	case	'C':		/* channel specification for display */
		...
		if(displaychan == 0){
			kprint(2, "emu: invalid channel specifier (-C): %q\n", cp);
			_exits("usage");
		}
		break;
	case 'S':
...
main(int argc, char *argv[])
{
  ...
  opt = nil; //getenv("EMU");
	...
	//if(vflag)
	//	kprint(1, "Inferno %s main (pid=%d) %s\n", VERSION, getpid(), opt);
...
emuinit(void *imod)
{
	...
	e = up->env;
//print("strlen %d strdup %s strrchr %s strcmp %d\n", strlen("abcdefghij"),strcpy(strdup("1234567"),"fd"), strrchr("abcdabcdef", 'd'), strcmp("1234567","456"));
kprint(2, "emuinit1: sizeof Proc %d pgrp %d fgrp %d egrp %d\n", sizeof(Proc), sizeof(Pgrp), sizeof(Fgrp), sizeof(Egrp));
	e->pgrp = newpgrp();
	e->fgrp = newfgrp(nil);
	e->egrp = newegrp();
	e->errstr = e->errbuf0;
...
kprint(2, "emuinit2: links & chandevinit Block=%d\n", sizeof(Block));
	links();
	chandevinit();
...
kprint(2, "emuinit3: init pgrp Bhdr=%d\n", sizeof(Bhdr));
...
kprint(2, "emuinit4: init cons Mnt=%d\n", sizeof(Mnt));
	if(kopen("#c/cons", OREAD) != 0)
		kprint(2, "failed to make fd0 from #c/cons: %r\n");
	kopen("#c/cons", OWRITE);
	kopen("#c/cons", OWRITE);

kprint(2, "emuinit5: bind Dir=%d\n", sizeof(Dir));
...
kprint(2, "emuinit6: init env Mhead=%d\n", sizeof(Mhead));
	if(cputype != nil)
...
kprint(2, "emuinit7: init complete\n", sizeof(Mhead));
