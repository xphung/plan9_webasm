...
struct
{
	...
	3,
	{
		...
		{ "image", 2,   32*1024*1024+256, 63, 4*1024*1024, 1, 31*1024*1024 },
	}
};
...
void*
vmalloc(size_t size, const char *type, char *data)	/* vebose malloc */
{
	void *v;
...
int hdrsize = BHDRSIZE + Npadlong*sizeof(ulong);
int asize = (size + hdrsize + mainmem->quanta) & ~(mainmem->quanta);
//kprint(2, "vmalloc: %d+%3.3d = %X -> %X ~ [%9.9s] by %s\n", hdrsize, size, v, (char *)v+asize, data, type);
	return v;
}

void*
vsmalloc(size_t size, const char *type, char *data)
{
	void *v;

	for(;;){
		v = vmalloc(size, type, data);
		if(v != nil)
			break;
		if(0)
...
void*
vkmalloc(size_t size, const char *type, char *data)
{
	void *v;
...
int hdrsize = BHDRSIZE + Npadlong*sizeof(ulong);
int asize = (size + hdrsize + mainmem->quanta) & ~(mainmem->quanta);
//kprint(2, "kmalloc: %d+%3.3d = %X -> %X ~ [%9.9s] by %s\n", hdrsize, size, v, (char *)v+asize, data, type);
	return v;
}
...
void
kfree(void *v, const char *type, char *data)
{
	Bhdr *b;

	if(v != nil) {
//kprint(2, "kfree : -%d-%3.3d = %X -> %X ~ [%9.9s] by %s\n", Npadlong*sizeof(ulong), BHDRSIZE, v, (char *)v+0+mainmem->quanta, data, type);
		if(Npadlong)
			v = (ulong*)v-Npadlong;
		D2B(b, v);
		ML(v, 0, 0);
...
{
	print("%s(%p): pool %s CORRUPT: %s at %p'%lux(magic=%lux)\n",
