...
devattach(int tc, char *spec)
{
	...
	if(spec == nil)
		spec = "";
	buf = vsmalloc(4+strlen(spec)+1, "devattach", spec);
	sprint(buf, "#%C%s", tc, spec);
	c->name = newcname(buf);
	kfree(buf, "devdetach", spec);
	return c;
}
...
devwalk(Chan *c, Chan *nc, char **name, int nname, Dirtab *tab, int ntab, Devgen *gen)
{
	...
	alloc = 0;
	wq = vsmalloc(sizeof(Walkqid)+(nname-1)*sizeof(Qid), "devwalk", c2name(c));
	if(waserror()){
...
//print("\rdev.walk: begin loop nname=%d\n", nname);
	for(j=0; j<nname; j++){
//print("dev.walk: begin iteration j=%d\n", j);
...
//print("dev.walk: check . & .. in n=%s\n", n);
    ...
    Accept:
		...
		for(i=0;; i++) {
			...
			Notfound:
				if(j == 0) {
					/* XP 2018-11-14: below is duplicate of waserror() exception catch code above */
					if(alloc && wq->clone!=nil) cclose(wq->clone);
					free(wq);
					return nil; //error(Enonexist);
        			}
